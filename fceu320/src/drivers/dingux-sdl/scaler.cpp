...
void upscale_320x240_bilinearish_clip(uint32_t* dst, uint8 *src, int width)
{
	...
	for (BlockY = 0; BlockY < 14; BlockY++)
	{
		...
		for (BlockX = 0; BlockX < 80; BlockX++)
		{
			...
			*(BlockDst + 320 *  1 * 2 + 0) = _4;
			uint16_t  _5 = palettetranslate[*(BlockSrc + 256 *  1 + 1 + 8)];
			*(BlockDst + 320 *  1 * 2 + 1) = Weight1_3( _4,  _5);
			uint16_t  _6 = palettetranslate[*(BlockSrc + 256 *  1 + 2 + 8)];
			*(BlockDst + 320 *  1 * 2 + 2) = Weight3_1( _5,  _6);
			*(BlockDst + 320 *  1 * 2 + 3) = _6;

			// -- Row 3 --
			uint16_t  _7 = palettetranslate[*(BlockSrc + 256 *  2     + 8)];
			*(BlockDst + 320 *  2 * 2 + 0) = _7;
			uint16_t  _8 = palettetranslate[*(BlockSrc + 256 *  2 + 1 + 8)];
			*(BlockDst + 320 *  2 * 2 + 1) = Weight1_3( _7, _8);
			uint16_t  _9 = palettetranslate[*(BlockSrc + 256 *  2 + 2 + 8)];
			*(BlockDst + 320 *  2 * 2 + 2) = Weight3_1(_8, _9);
			*(BlockDst + 320 *  2 * 2 + 3) = _9;

			// -- Row 4 --
			uint16_t _10 = palettetranslate[*(BlockSrc + 256 *  3     + 8)];
			*(BlockDst + 320 *  3 * 2 + 0) = Weight1_3( _7, _10);
			uint16_t _11 = palettetranslate[*(BlockSrc + 256 *  3 + 1 + 8)];
			*(BlockDst + 320 *  3 * 2 + 1) = Weight1_3(Weight1_3( _7, _8), Weight1_3(_10, _11));
			uint16_t _12 = palettetranslate[*(BlockSrc + 256 *  3 + 2 + 8)];
			*(BlockDst + 320 *  3 * 2 + 2) = Weight1_3(Weight3_1(_8, _9), Weight3_1(_11, _12));
			*(BlockDst + 320 *  3 * 2 + 3) = Weight1_3(_9, _12);

			// -- Row 5 --
			uint16_t _13 = palettetranslate[*(BlockSrc + 256 *  4     + 8)];
			*(BlockDst + 320 *  4 * 2 + 0) = Weight1_3(_10, _13);
			uint16_t _14 = palettetranslate[*(BlockSrc + 256 *  4 + 1 + 8)];
			*(BlockDst + 320 *  4 * 2 + 1) = Weight1_3(Weight1_3(_10, _11), Weight1_3(_13, _14));
			uint16_t _15 = palettetranslate[*(BlockSrc + 256 *  4 + 2 + 8)];
			*(BlockDst + 320 *  4 * 2 + 2) = Weight1_3(Weight3_1(_11, _12), Weight3_1(_14, _15));
			*(BlockDst + 320 *  4 * 2 + 3) = Weight1_3(_12, _15);

			// -- Row 6 --
			uint16_t _16 = palettetranslate[*(BlockSrc + 256 *  5     + 8)];
			*(BlockDst + 320 *  5 * 2 + 0) = Weight1_3(_13, _16);
			uint16_t _17 = palettetranslate[*(BlockSrc + 256 *  5 + 1 + 8)];
			*(BlockDst + 320 *  5 * 2 + 1) = Weight1_3(Weight1_3(_13, _14), Weight1_3(_16, _17));
			uint16_t _18 = palettetranslate[*(BlockSrc + 256 *  5 + 2 + 8)];
			*(BlockDst + 320 *  5 * 2 + 2) = Weight1_3(Weight3_1(_14, _15), Weight3_1(_17, _18));
			*(BlockDst + 320 *  5 * 2 + 3) = Weight1_3(_15, _18);

			// -- Row 7 --
			uint16_t _19 = palettetranslate[*(BlockSrc + 256 *  6     + 8)];
			*(BlockDst + 320 *  6 * 2 + 0) = Weight1_3(_16, _19);
			uint16_t _20 = palettetranslate[*(BlockSrc + 256 *  6 + 1 + 8)];
			*(BlockDst + 320 *  6 * 2 + 1) = Weight1_3(Weight1_3(_16, _17), Weight1_3(_19, _20));
			uint16_t _21 = palettetranslate[*(BlockSrc + 256 *  6 + 2 + 8)];
			*(BlockDst + 320 *  6 * 2 + 2) = Weight1_3(Weight3_1(_17, _18), Weight3_1(_20, _21));
			*(BlockDst + 320 *  6 * 2 + 3) = Weight1_3(_18, _21);

			// -- Row 8 --
			uint16_t _22 = palettetranslate[*(BlockSrc + 256 *  7     + 8)];
			*(BlockDst + 320 *  7 * 2 + 0) = Weight1_3(_19, _22);
			uint16_t _23 = palettetranslate[*(BlockSrc + 256 *  7 + 1 + 8)];
			*(BlockDst + 320 *  7 * 2 + 1) = Weight1_3(Weight1_3(_19, _20), Weight1_3(_22, _23));
			uint16_t _24 = palettetranslate[*(BlockSrc + 256 *  7 + 2 + 8)];
			*(BlockDst + 320 *  7 * 2 + 2) = Weight1_3(Weight3_1(_20, _21), Weight3_1(_23, _24));
			*(BlockDst + 320 *  7 * 2 + 3) = Weight1_3(_21, _24);

			// -- Row 9 --
			uint16_t _25 = palettetranslate[*(BlockSrc + 256 *  8     + 8)];
			*(BlockDst + 320 *  8 * 2 + 0) = Weight1_1(_22, _25);
			uint16_t _26 = palettetranslate[*(BlockSrc + 256 *  8 + 1 + 8)];
			*(BlockDst + 320 *  8 * 2 + 1) = Weight1_1(Weight1_3(_22, _23), Weight1_3(_25, _26));
			uint16_t _27 = palettetranslate[*(BlockSrc + 256 *  8 + 2 + 8)];
			*(BlockDst + 320 *  8 * 2 + 2) = Weight1_1(Weight3_1(_23, _24), Weight3_1(_26, _27));
			*(BlockDst + 320 *  8 * 2 + 3) = Weight1_1(_24, _27);

			// -- Row 10 --
			uint16_t _28 = palettetranslate[*(BlockSrc + 256 *  9     + 8)];
			*(BlockDst + 320 *  9 * 2 + 0) = Weight3_1(_25, _28);
			uint16_t _29 = palettetranslate[*(BlockSrc + 256 *  9 + 1 + 8)];
			*(BlockDst + 320 *  9 * 2 + 1) = Weight3_1(Weight1_3(_25, _26), Weight1_3(_28, _29));
			uint16_t _30 = palettetranslate[*(BlockSrc + 256 *  9 + 2 + 8)];
			*(BlockDst + 320 *  9 * 2 + 2) = Weight3_1(Weight3_1(_26, _27), Weight3_1(_29, _30));
			*(BlockDst + 320 *  9 * 2 + 3) = Weight3_1(_27, _30);

			// -- Row 11 --
			uint16_t _31 = palettetranslate[*(BlockSrc + 256 * 10     + 8)];
			*(BlockDst + 320 * 10 * 2 + 0) = Weight3_1(_28, _31);
			uint16_t _32 = palettetranslate[*(BlockSrc + 256 * 10 + 1 + 8)];
			*(BlockDst + 320 * 10 * 2 + 1) = Weight3_1(Weight1_3(_28, _29), Weight1_3(_31, _32));
			uint16_t _33 = palettetranslate[*(BlockSrc + 256 * 10 + 2 + 8)];
			*(BlockDst + 320 * 10 * 2 + 2) = Weight3_1(Weight3_1(_29, _30), Weight3_1(_32, _33));
			*(BlockDst + 320 * 10 * 2 + 3) = Weight3_1(_30, _33);

			// -- Row 12 --
			uint16_t _34 = palettetranslate[*(BlockSrc + 256 * 11     + 8)];
			*(BlockDst + 320 * 11 * 2 + 0) = Weight3_1(_31, _34);
			uint16_t _35 = palettetranslate[*(BlockSrc + 256 * 11 + 1 + 8)];
			*(BlockDst + 320 * 11 * 2 + 1) = Weight3_1(Weight1_3(_31, _32), Weight1_3(_34, _35));
			uint16_t _36 = palettetranslate[*(BlockSrc + 256 * 11 + 2 + 8)];
			*(BlockDst + 320 * 11 * 2 + 2) = Weight3_1(Weight3_1(_32, _33), Weight3_1(_35, _36));
			*(BlockDst + 320 * 11 * 2 + 3) = Weight3_1(_33, _36);

			// -- Row 13 --
			uint16_t _37 = palettetranslate[*(BlockSrc + 256 * 12     + 8)];
			*(BlockDst + 320 * 12 * 2 + 0) = Weight3_1(_34, _37);
			uint16_t _38 = palettetranslate[*(BlockSrc + 256 * 12 + 1 + 8)];
			*(BlockDst + 320 * 12 * 2 + 1) = Weight3_1(Weight1_3(_34, _35), Weight1_3(_37, _38));
			uint16_t _39 = palettetranslate[*(BlockSrc + 256 * 12 + 2 + 8)];
			*(BlockDst + 320 * 12 * 2 + 2) = Weight3_1(Weight3_1(_35, _36), Weight3_1(_38, _39));
			*(BlockDst + 320 * 12 * 2 + 3) = Weight3_1(_36, _39);

			// -- Row 14 --
			uint16_t _40 = palettetranslate[*(BlockSrc + 256 * 13     + 8)];
			*(BlockDst + 320 * 13 * 2 + 0) = Weight3_1(_37, _40);
			uint16_t _41 = palettetranslate[*(BlockSrc + 256 * 13 + 1 + 8)];
			*(BlockDst + 320 * 13 * 2 + 1) = Weight3_1(Weight1_3(_37, _38), Weight1_3(_40, _41));
			uint16_t _42 = palettetranslate[*(BlockSrc + 256 * 13 + 2 + 8)];
			*(BlockDst + 320 * 13 * 2 + 2) = Weight3_1(Weight3_1(_38, _39), Weight3_1(_41, _42));
			*(BlockDst + 320 * 13 * 2 + 3) = Weight3_1(_39, _42);

			// -- Row 15 --
			*(BlockDst + 320 * 14 * 2 + 0) = _40;
			*(BlockDst + 320 * 14 * 2 + 1) = Weight1_3(_40, _41);
			*(BlockDst + 320 * 14 * 2 + 2) = Weight3_1(_41, _42);
			*(BlockDst + 320 * 14 * 2 + 3) = _42;

			// -- Row 16 --
			uint16_t _43 = palettetranslate[*(BlockSrc + 256 * 14     + 8)];
			*(BlockDst + 320 * 15 * 2 + 0) = _43;
			uint16_t _44 = palettetranslate[*(BlockSrc + 256 * 14 + 1 + 8)];
			*(BlockDst + 320 * 15 * 2 + 1) = Weight1_3(_43, _44);
			uint16_t _45 = palettetranslate[*(BlockSrc + 256 * 14 + 2 + 8)];
			*(BlockDst + 320 * 15 * 2 + 2) = Weight3_1(_44, _45);
			*(BlockDst + 320 * 15 * 2 + 3) = _45;

			// -- Row 17 --
			uint16_t _46 = palettetranslate[*(BlockSrc + 256 * 15     + 8)];
			*(BlockDst + 320 * 16 * 2 + 0) = _46;
			uint16_t _47 = palettetranslate[*(BlockSrc + 256 * 15 + 1 + 8)];
			*(BlockDst + 320 * 16 * 2 + 1) = Weight1_3(_46, _47);
			uint16_t _48 = palettetranslate[*(BlockSrc + 256 * 15 + 2 + 8)];
			*(BlockDst + 320 * 16 * 2 + 2) = Weight3_1(_47, _48);
			*(BlockDst + 320 * 16 * 2 + 3) = _48;
