...
function SetGlobal()   --设置游戏内部使用的全程变量
   ...
   CC.KRP=CONFIG.DataPath .. "kdef.grp";   --kdef.grp的路径
   CC.KDX=CONFIG.DataPath .. "kdef.idx";   --kdef.idx的路径
   CC.TRP=CONFIG.DataPath .. "talk.grp";   --talk.grp的路径
   CC.TDX=CONFIG.DataPath .. "talk.idx";   --talk.idx的路径   

   --CC.KRP="ms0:psp/game/kyscczr/data/kdef.grp";   --kdef.grp的路径
   --CC.KDX="ms0:psp/game/kyscczr/data/kdef.idx";   --kdef.idx的路径
   --CC.TRP="ms0:psp/game/kyscczr/talk.grp";   --talk.grp的路径
   --CC.TDX="ns0:psp/game/kyscczr/talk.idx";   --talk.idx的路径
...
function SelectThing(thing,thingnum)    --显示物品菜单
	...
	local dx=(CC.ScreenW-w)/2;
	local dy=(CC.ScreenH-h-2*(CC.ThingFontSize+2*CC.MenuBorderPixel+5))/2-25;    --local dy=(CC.ScreenH-h-2*(CC.ThingFontSize+2*CC.MenuBorderPixel+5))/2-50;
            ...
            for x=0,xnum-1 do
                ...
                if x==cur_x and y==cur_y then
                    ...
                    if thing[id]>=0 then
						...
						local mx,my=dx+4*myfont+12,y3_2+8--130,365
						local myflag=0
						...
						local function drawitem(ss,str,news)
							...
							local mylen=myfont*string.len(mys)/2+24
						...
						if myThing['练出武功']>0 then
							...
							mx=mx+myfont*string.len(kfname)/2+24
						end
						...
						if mx~=dx or my~=y3_2+2 then
							DrawStrBox(dx,y3_2+8,' 效果:',C_RED,myfont)
						end
						end
...
function oldCallEvent1(eventnum)     --执行旧的事件函数
