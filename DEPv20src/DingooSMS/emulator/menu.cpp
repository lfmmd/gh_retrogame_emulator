...
{
	if(isSMS()) 
		return video::tex_load(png_logo_sms, png_logo_sms_size);
	if(isGG()) 
		return video::tex_load(png_logo_gg, png_logo_gg_size);
	if(isColeco()) 
		return video::tex_load(png_logo_coleco, png_logo_coleco_size);
}

gfx_texture* emul::menu_load_gamepad()
{
	if(isSMS()) 
		return video::tex_load(png_gamepad_sms, png_gamepad_sms_size);
	if(isGG()) 
		return video::tex_load(png_gamepad_gg, png_gamepad_gg_size);
	if(isColeco()) 
		return video::tex_load(png_gamepad_coleco, png_gamepad_coleco_size);
}
