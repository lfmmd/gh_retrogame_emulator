...
bool core::init()
{
	backdrop = hw::video::tex_load(png_backdrop, png_backdrop_size);
	return true;
}
...
bool core::load_rom()
{
	if(system_load_rom(fsys::getGameFilePath())) {
...
void core::emulate_frame(bool& withRendering, int soundBuffer)
{
	...
	withRendering = system_frame(!withRendering);
...
void core::emulate_for_screenshot(int& console_scr_w, int& console_scr_h)
{
	...
	bitmap.data = (uint8_t*)(hw::video::get_framebuffer() + VBuffOffset);
	system_frame(0); if(sms.glasses_3d) if(!system_frame(0)) system_frame(0);
...
static void InitVideoSystem()
{
	if(settings.fullScreen) {
		...
		if(isGG()) {
			hw::video::bind_framebuffer();
			hw::video::draw_tex_transp(0, 0, backdrop);
			hw::video::term_framebuffer();
			hw::video::flip(0);
			hw::video::bind_framebuffer();
			hw::video::draw_tex_transp(0, 0, backdrop);
			hw::video::term_framebuffer();
...
static void UpdateInput()
{
	...
	#define KEYBOARD coleco.keypad[0]
	...
	if(controls)
	{
		...
		if(isColeco()) {
			if (controls & hw::input::get_mask(settings.pad_config[ 6])) GAMEPAD |= GAMEPAD_1;
			if (controls & hw::input::get_mask(settings.pad_config[ 7])) GAMEPAD |= GAMEPAD_2;
			if (controls & hw::input::get_mask(settings.pad_config[ 8])) KEYBOARD = KEYBOARD_1;
			if (controls & hw::input::get_mask(settings.pad_config[ 9])) KEYBOARD = KEYBOARD_2;
			if (controls & hw::input::get_mask(settings.pad_config[10])) KEYBOARD = KEYBOARD_S;
			if (controls & hw::input::get_mask(settings.pad_config[11])) KEYBOARD = KEYBOARD_P;
			//
        } else 
		if(isGG()) {
	...
	if(isColeco() && hw::video::has_osd_msg() < 30) 
	{
		if (controls & hw::input::get_mask(settings.pad_config[12])) {
			if(keyboardKeyVisible) if(--keyboardKeyIndex < 0) keyboardKeyIndex = 11;
			printSelectedKey();
		} else
		if (controls & hw::input::get_mask(settings.pad_config[13])) {
			if(keyboardKeyVisible) if(++keyboardKeyIndex > 11) keyboardKeyIndex = 0;
			printSelectedKey();
		}
		if(keyboardKeyVisible && !hw::video::has_osd_msg())	{
			keyboardKeyVisible = false;
			KEYBOARD = keyboardKeyIndex;
		}
	}
}
