...
void menu::init(const char* romName, gfx_color romNameColor)
{
	...
	mMenuRomName[MAX_MENU_ITEM_CHARS] = 0;
...
{
	//	draw tiled background
	int x, y, font_h = video::font_h(video::font_get());
	render_tiled_bg();
	...
	x = (HW_SCREEN_WIDTH - video::font_w(video::font_get(), mMenuTitle)) >> 1;
	y = (16 - font_h) >> 1;
	...
	if(COLOR_FOOTER != COLOR_TRANSPARENT) {
		if(HALF_TRANSP_FOOTER)
			video::mult_rect(0, HW_SCREEN_HEIGHT - 16, HW_SCREEN_WIDTH, 16, COLOR_FOOTER);
		else
			video::fill_rect(0, HW_SCREEN_HEIGHT - 16, HW_SCREEN_WIDTH, 16, COLOR_FOOTER);
	}
	if(COLOR_HF_LINE != COLOR_TRANSPARENT) video::fill_rect(0, HW_SCREEN_HEIGHT - 16 - 2, HW_SCREEN_WIDTH, 2, COLOR_HF_LINE);
	x = (HW_SCREEN_WIDTH - video::font_w(video::font_get(), mMenuRomName)) >> 1;
	y = HW_SCREEN_HEIGHT - 16 + ((16 - font_h) >> 1);
	render_text(x, y, mMenuRomName, mRomNameColor);
}

void menu::render_menu(int count, int focus)
{
	mMenuSmooth = mMenuSmooth * 7 + (focus << 8);
	mMenuSmooth >>= 3;
	...
	//	draw selector
	int y = 112 - (16 - mMenuInterval);
	int x = 16 - mMenuInterval;
	int w = mMenuInterval * (HW_SCREEN_WIDTH / 16);
	if(COLOR_SELECTOR_INSIDE != COLOR_TRANSPARENT) {
		if(HALF_TRANSP_SELECTOR)
			video::mult_rect(x, y, w, 16, COLOR_SELECTOR_INSIDE);
		else
			video::fill_rect(x, y, w, 16, COLOR_SELECTOR_INSIDE);
	}
	if(COLOR_SELECTOR_EDGING != COLOR_TRANSPARENT) render_rect(x, y, w, 16, COLOR_SELECTOR_EDGING);
	...
	//	draw menu items
	int font_h = video::font_h(video::font_get());
	for (int i = 0; i < count; i++)
	{
		y = 112 + 4 + (i * mMenuInterval) - (mMenuSmooth / mMenuInterval);
		if (y <= 16 + 2 || y >= HW_SCREEN_HEIGHT - (16 + 2) - font_h) continue;
...
void menu::render_stateshot(int x, int y, int w, int h, int mode, int slot, gfx_color* stateCurrent, gfx_color* stateSaved)
{
	...
	if(mode == 0) {
		...
		sprintf(slotText, "\x11 Slot %d \x10", slot);
	...
	x += (w  - video::font_w(video::font_get(), slotText)) >> 1;
	y += (16 - video::font_h(video::font_get())) >> 1;
...
void fsys::setDirectories(cstr_t emulPath, cstr_t gameFilePath)
{
