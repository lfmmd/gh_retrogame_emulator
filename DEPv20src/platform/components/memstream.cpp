...
void MemStream::close()
{
	...
	if(memInternal) {
		printf("internal buff used\n");
		if(isDynamic) {
			printf("free data in internal buff\n");
			memInternal->free(); 
		} else {
			printf("zero buffer pointer in internal buff\n");
			memInternal->buffAddr = NULL;
		}
		printf("delete internal buff object\n");
		delete memInternal;
		memInternal = NULL;
	}
...
bool MemStream::ensureCapacity(size_t writeSize)
{
    ...
    if(needSize <= memBuffer->buffSize) return true;
