...
bool state_use_memory(MemBuffer* memBuffer)
{
	if(ioMode == IO_MODE_NONE && memBuffer)	{
		...
		printf("state_use_memory: OK\n");
		return true; 
	}
	printf("state_use_memory: FAILED\n");
	return false;
}
...
bool state_use_file(cstr_t filePath)
{
	if(ioMode == IO_MODE_NONE && filePath)	{
		...
		ioMode = IO_MODE_FILE;
		printf("state_use_file: OK\n");
		return true; 
	}
	printf("state_use_file: FAILED\n");
	return false;
}
...
bool state_fopen(cstr_t mode)
{
	...
	if(ioMode == IO_MODE_MEMORY) 
	{
		...
		if(mode[0] == 'r')
		{
			if(dataStream.open(memBuffer))
			{
				rwMode = RW_MODE_READ;
				result = true;
				printf("state_fopen: memory: read: OK\n");
			}
		} else
		if(mode[0] == 'w')
		{
			...
			if(dataStream.open(memBuffer))
			{
				...
				result = true;
				printf("state_fopen: memory: write: OK\n");
			}
		}
		if(!result) printf("state_fopen: memory: FAILED\n");
	} else 
	if(ioMode == IO_MODE_FILE)
	{
		...
		if(mode[0]=='r' && state_is_correct(filePath) && load_zip_file(filePath))
		{
			...
			rwMode = RW_MODE_READ;
			result = true;
			printf("state_fopen: file: read: OK\n");
		} else
		if(mode[0] == 'w' && dataStream.open())
		{
			...
			rwMode = RW_MODE_WRITE;
			result = true;
			printf("state_fopen: file: write: OK\n");
		}
		if(!result) printf("state_fopen: file: FAILED\n");
	}
	if(!result)
	{
		ioMode = IO_MODE_NONE;
...
int state_fclose()
{
	if(ioMode == IO_MODE_FILE && rwMode == RW_MODE_WRITE)
	{
		...
		zipFileName[0] = 0;
		printf("state_fclose: save zip file\n");
	}
