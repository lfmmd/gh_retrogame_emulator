...
void core::emulate_frame(bool& withRendering, int soundBuffer)
{
	...
	FCEUI_Emulate(&nes_gfx, &nes_snd, &nes_snd_count, !withRendering);
	if(nes_gfx) {
...
{
	FCEUI_Emulate(&nes_gfx, &nes_snd, &nes_snd_count, 0);
...
bool core::save_load_state(cstr_t filePath, MemBuffer* memBuffer, bool load)	//	TODO: error checking
{
	...
	{
		if(load) FCEUI_LoadState(filePath); else FCEUI_SaveState(filePath);
	} else
